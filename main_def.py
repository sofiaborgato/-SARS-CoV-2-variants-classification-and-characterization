...
path=os.getcwd() #path of the working directory

#The git repository with the code and the data is cloned into the working directory, if it's not already present
...
if in_path != '0':
	...
	#stats: statistics of the aligned genomes mutations 
	#mutation list :characterization of each mutation found 
    key_mut = key_mutations(mutation_list, len(stats)) #for each line evaluate the most frequent mutations
    export_data(stats, genomes_aligned,key_mut, name = "new") #store the data in the output folder 

#Demo of supervised classification
...
#Demo of unsupervised classification    
...
shutil.copytree(os.path.join(path,'Bioinformatic_project/Output'),os.path.join(out_path,out_name))  #moves the output folder to the desired path
shutil.rmtree(os.path.join(path,'Bioinformatic_project')) #remove the working folder
